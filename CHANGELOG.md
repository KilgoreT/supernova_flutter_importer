...
- Исправлена ошибка "Template not found" в конце Base файлов
- Заменен некорректный вызов renderTemplate() на прямую компиляцию Handlebars для templateBaseClass
- Добавлена регистрация частичного шаблона dart_color_field перед компиляцией
- Удален неиспользуемый импорт renderTemplate

### 🔧 Changed

- Добавлена поддержка корректного пути basePath в импортах для Master Class
- Импорты теперь генерируются в формате: `import 'package:ui_kit_litnet_audio/kw/colors/green.dart';`
- Добавлен признак isRootClass в шаблон templateBaseClass для публичных/приватных конструкторов
- Корневые токен-группы теперь имеют публичные конструкторы, вложенные классы - приватные

### 📋 Technical Notes

- templateBaseClass() теперь возвращает строку шаблона для прямой компиляции Handlebars
- Все функции используют Handlebars.compile() напрямую вместо renderTemplate()
- Добавлена автоматическая регистрация dart_color_field как частичный шаблон
- Исправлена логика генерации импортов с учетом basePath из конфигурации

## 1.0.15

### 🔧 Changed

- КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Восстановлена правильная структура unified mode
- AppColors теперь создается как отдельный файл только с ссылками на классы
- Каждый цветовой класс (Basic, Neutral, etc.) теперь создается в отдельном файле
- Убрана логика объединения всех классов в один файл AppColors.dart
- Теперь unified mode работает как ожидается: отдельные файлы + корневой AppColors

### 📋 Technical Notes

- generateUnifiedColors() теперь создает сначала отдельные файлы для каждого класса
- Затем создает AppColors.dart только с полями-ссылками на эти классы
- Используется isUnifiedMode = false для отдельных файлов (static поля в корневых классах)
- AppColors остается корневым классом со static полями

## 1.0.14

### 🔧 Changed

- Подготовлены все исходники для правильной работы логики статических полей
- Исходный код в src/ теперь содержит правильную логику с isRootClass
- При следующей сборке dist/build.js будет содержать правильную логику
- Убеждаемся, что Node.js сборка создаст корректный dist файл

### 📋 Technical Notes

- src/core/render/renderer.ts: шаблон использует {{#isStatic}}static {{/isStatic}}
- src/content/color.ts: использует isRootClass для определения статичности
- Unified mode: AppColors корневой (static), Basic/Neutral вложенные (не static)
- Normal mode: Basic/Neutral корневые (static), BasicText вложенные (не static)
